...
Security:
* mcollective['securityprovider'] - the security plugin to use (default: "psk")
* mcollective['psk'] - the key for the "psk" security provider.

Message Broker connection settings:
* mcollective['connector'] - which connection plugin to use (default: "stomp")

STOMP service connection details (used by "stomp" and "activemq" plugins):
...
* mcollective['plugin_conf'] - per-plugin configuration files
...
packages, set node['mcollective']['enable_puppetlabs_repo'] to false.
If you need to make more extensive changes, write your own recipes and
update node['mcollective']['install_recipe'] to use them instead.
